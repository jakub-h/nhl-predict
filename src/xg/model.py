...
def fit_gradient_boosting(features_train, labels_train):
    ...
    clf = GradientBoostingClassifier(n_estimators=500, learning_rate=.1, random_state=42)

    print("Fitting Gradient Boosting Classifier")

    # Tune hyperparameters
    cv_clf = GridSearchCV(estimator=clf, param_grid=param_grid, cv=10, n_jobs=-1, verbose=2)
    ...
    # Save model
    pickle.dump(cv_clf, open("models/gmb_multiclass_xg.pkl", 'wb'))
...
def fit_random_forest(features_train, labels_train):
    ...
    # Save model
    pickle.dump(cv_clf, open("models/rf_rebounds_xg.pkl", 'wb'))
    return cv_clf
...
def fit_logistic(features_train, labels_train):
    ...
    # Save model
    pickle.dump(cv_clf, open("models/logistic_xg.pkl", 'wb'))
